...
/**
 * Dada a lista de transações, agrupe-as pelo seu respectivo canal (ATM,
 * Online).
 * Exemplo
 * Entrada: (Lista padrão Aula 04)
 * Saída:
 * ATM: [TX000001, TX000002]
 * Online: [TX000003, TX000004, TX000005]
 */
public class AgrupamentoCanais {

    public static void main(String[] args) {
        Map<Channel, List<Transaction>> txAgrupadasPorCanal = Stream.of(
        ...
        for (Channel channel: txAgrupadasPorCanal.keySet()) {
            List<String> transactions = txAgrupadasPorCanal.get(channel)
                                                            .stream()
                                                            .map(Transaction::transactionId).toList();
            System.out.println("%s: %s".formatted(channel, transactions));
        }
